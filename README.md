...
- Email/password sign-in and registration, plus a one-click demo account
...
Route Handlers under `app/api/` (e.g. `auth/login`, `auth/refresh`, `private/shift/bulks`, `private/cwh/eod-report`) forward the request to the backend, read/attach cookies, and return a normalized response. This pattern is used because, based on the implementation:
...
- Sessions are represented by three cookies set by the backend and read on the Next.js server: `accessToken`, `refreshToken`, and `deviceId` (`lib/auth/session.ts`, `proxy.ts`).
...
- **Token refresh** happens at `/refresh`, which calls `/api/auth/refresh`; this Route Handler asks the backend to issue a new access token using the refresh cookie and forwards the new `Set-Cookie` headers, or clears cookies on failure.
...
3. Once a cycle exists, shifts for that cycle are loaded and rendered client-side via a reducer (`shiftCalculator.reducer.ts`) that tracks draft vs. saved shifts.
4. Users can add, edit, delete, and duplicate-to-next-day shifts entirely in local state before saving.
5. Deleted shift IDs are tracked separately so they can be removed from the backend on save.
6. "Save" sends all draft shifts, deleted IDs, and the cycle range in one bulk request to `/api/private/shift/bulks`.
7. After a successful save, the backend-calculated daily summaries and estimated gross pay are refreshed and displayed — payroll math is performed by the ASP.NET Core backend, not duplicated as trusted business logic in the frontend.
...
| `/auth/register` | Account registration | Public |
